...
@Component({
  ...
  styleUrls: ['./closed-job.component.scss']
})
export class ClosedJobComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  openWorksheet(job){
    ...
    console.log(obj);
    this.isMailSent = true;
    this.emailService.openWorkSheet(job)
      .subscribe(
        data => {
          if (data.status == 0) {
            this.toastr.error(data.message)
        ...
        data => {
          ...
          if (data.status == 0) {
            ...
            this.attachmentList = list.map(item =>
              // new ImageItem({ src: item.fileName, thumb: item.fileName })
  ...
  updateRepairDetails() {
    ...
    this.dataList.forEach(element => {
      ...
      if (element.type != 'service') {
        ...
        str = { ID: element.ID, productID: null, jobID: this.jobObj.id,invoiceID: this.invoiceData.invoiceID, name: element.name, quantity: element.quantity, price: element.price }
      }
      list.push(str);
        ...
        .subscribe(
          data => {
            if (data.data.status == 0) {
              ...
              this.listPro = { product: this.productGroup.value.productName, price: this.productGroup.value.sellPrice, quanity: 1 }
            ...
            if (data.result.length > 0) {
              ...
              data.result.forEach(element => {
                ...
                if (element.productID == null && element.productID == undefined) {
                  str = { ID: element.id, id: null, jobID: element.jobID,invoiceID: this.invoiceData.invoiceID, name: element.name, service: "1", quantity: element.quantity, price: element.price }

                } else {
                  str = { ID: element.id, id: element.productID,invoiceID: this.invoiceData.invoiceID, jobID: element.jobID, name: element.name, quantity: element.quantity, price: element.price,unitPrice:element.unitPrice  }
