...
import { AssignedService } from 'src/app/modules/auth/_services/assigned.service';
...
export class DashboardComponent implements OnInit {
  ...
  public filteredtypeMulti: ReplaySubject<types[]> = new ReplaySubject<types[]>(1);

  public typeMultiFilterCtrl: FormControl = new FormControl();
  private typesVal: types[] = [
    { label: 'Custom Daterange',value:"customdaterange"},
    { label: 'All Time',value:"alltime"},
    { label: 'This Week',value:"thisweek"},
    { label: 'This Month',value:"thismonth"},
    { label: 'This Year',value:"v"},
  ...
  constructor(private fb: FormBuilder,
    public proService: ProductPurchaseService,
  ...
  ngOnInit(): void {
    ...
    const sb = this.assignedService.isLoading$.subscribe(res => this.isLoading = res);
    this.grouping = this.assignedService.grouping;
    this.paginator = this.assignedService.paginator;
    this.sorting = this.assignedService.sorting;
    ...
    this.paymentTypeMultiFilterCtrl.valueChanges
      ...
      .subscribe(() => {
        ...
        this.filteredtypeMulti.next(this.typesVal.slice());
  ...
  search(searchTerm: string) {
    ...
    this.assignedService.patchState({ searchTerm: searchTerm.toLocaleLowerCase() });
  }
  onSelect() {
      ...
      .subscribe(async data => {
          ...
          if (data.status == 0) {
            ...
            this.attachmentList = list.map(item =>
              ...
              // new ImageItem({ src:location.origin + '/'+ item.fileName, thumb:location.origin + '/'+ item.fileName, id: item.id })
 ...
 orgPrice1:number = 0;
  ...
  //       if (this.repairAssig.quantity >= this.totalQty) {
  ...
  //     }
  //     console.log(this.repairAssig.price)
  //   }
  //   // this.cdr.markForCheck();
  ...
  async getProducts_ServiceByJobID(item) {
    var data = {
      jobID: item
    }
            ...
            if (data.result.length > 0) {
              ...
              data.result.forEach(element => {
                ...
                if (element.productID == null || element.productID == undefined) {
                  str = { ID: element.id, id: element.serviceID, jobID: element.jobID, invoiceID: this.invoiceData.invoiceID, name: element.name, service: "1", quantity: element.quantity, price: element.price }

                } else {
                  str = { ID: element.id, id: element.productID, jobID: element.jobID, invoiceID: this.invoiceData.invoiceID, name: element.name, quantity: element.quantity, price: element.price,unitPrice:element.unitPrice  }
  ...
  sort(column: string) {
    ...
    this.assignedService.patchState({ sorting });
  }
  filterCustomer() {
    if (!this.paymentVal) {
      return;
  ...
  filterTypes() {
    ...
    this.filteredtypeMulti.next(
      this.typesVal.filter(item => item.label.toLowerCase().indexOf(search) > -1)
    );
  }
  // pagination
  paginate(paginator: PaginatorState) {
    this.assignedService.patchState({ paginator });
  }
...
isProductManualControlTouched(controlName): boolean {
  ...
  noData = true;
  getNotification() {
    this.proService.getProductNotification().subscribe((res: any) => {
      if (res.success) {
        this.noData = false;
        for (let i in res.result) {

          let countPer = res.result[i].availableStock * 100 / res.result[i].minimumStock;
          let data = {
            pName: res.result[i].name,
            avilStock: res.result[i].availableStock,
            count: countPer
          }
  ...
  createChart() {
    ...
    this.canvas2 = document.querySelector("#myChart2")
  ...
  getDashboardCount(val) {
    this.reportService.getDashboardCount(val).subscribe((res: any) => {
      if (res.success) {
        this.jobBook = res.result[0].alljobsCount;
        this.avgRepairPrice = res.result[0].avgPrice
        this.avgRepairTime = res.result[0].avgDays
        this.lateJob = res.result[0].lateJobs;
        this.completedJob = res.result[0].jobsCompleted;
        this.returnedJob = res.result[0].jobsReturned;
        this.openJob = res.result[0].jobsOpen;
        this.totalIncome = res.result[0].totalIncome;
        this.getDashboardCountPre(val)
      }
    })
  ...
  jobBookPre: any;
  avgRepairPricePre: any;
  avgRepairTimePre: any;
  lateJobPre: any;
  completedJobPre: any;
  returnedJobPre: any;
  openJobPre: any;
  totalIncomePre: any;
  getDashboardCountPre(val) {
    this.reportService.getDashboardCountPre(val).subscribe((res: any) => {
      if (res.success) {
        this.jobBookPre = res.result[0].alljobsCountPre;
        this.avgRepairPricePre = res.result[0].avgPricePre
        this.avgRepairTimePre = res.result[0].avgDaysPre
        this.lateJobPre = res.result[0].lateJobsPre;
        this.completedJobPre = res.result[0].jobsCompletedPre;
        this.returnedJobPre = res.result[0].jobsReturnedPre;
        this.openJobPre = res.result[0].jobsOpenPre;
        this.totalIncomePre = res.result[0].totalIncomePre;
     ...
     jobBookavg:any=0;
     avgRepairPriceavg:any = 0;
     avgRepairTimeavg:any = 0;
     lateJobavg:any = 0;
     completedJobavg:any = 0;
     returnedJobavg:any = 0;  
     totalIncomeavg:any = 0;
  async getTotalAverage(){
    var jobBookavg = ( Number(this.jobBook) - Number(this.jobBookPre) ) / Number(this.jobBookPre) * 100;
    var avgRepairPriceavg = (Number(this.avgRepairPrice) - Number(this.avgRepairPricePre)) / Number(this.avgRepairPricePre)   * 100;
    var avgRepairTimeavg = (Number(this.avgRepairTime) - Number(this.avgRepairTimePre)) / Number(this.avgRepairTimePre)   * 100;
    var lateJobavg = (Number(this.lateJob) - Number(this.lateJobPre)) / Number(this.lateJobPre) * 100;
    var completedJobavg = (Number(this.completedJob) - Number(this.completedJobPre)) / Number(this.completedJobPre)   * 100;
    var returnedJobavg = (Number(this.returnedJob) - Number(this.returnedJobPre)) / Number(this.returnedJobPre) * 100;
    var totalIncomeavg = (Number(this.totalIncome) - Number(this.totalIncomePre)) / Number(this.totalIncomePre) * 100;
    
     if( Number.isNaN(jobBookavg) || jobBookavg == Infinity){
     
      this.jobBookavg= 0 
     }else{
      this.jobBookavg= jobBookavg.toFixed(2) 
     }
     if(Number.isNaN(avgRepairPriceavg)|| avgRepairPriceavg == Infinity){
      
      this.avgRepairPriceavg= 0 
    
     }else{
      this.avgRepairPriceavg= avgRepairPriceavg.toFixed(2)
     }

     if(Number.isNaN(avgRepairTimeavg) || avgRepairTimeavg == Infinity){
  
      this.avgRepairTimeavg= 0 
     }else{
      this.avgRepairTimeavg= avgRepairTimeavg.toFixed(2) 
     }
     if(Number.isNaN(lateJobavg) || lateJobavg == Infinity){
      
      this.lateJobavg= 0 
     }else{
      this.lateJobavg= lateJobavg.toFixed(2) 
     }
     if(Number.isNaN(completedJobavg ) || completedJobavg == Infinity){
      
      this.completedJobavg= 0 
     }else{
      this.completedJobavg= completedJobavg.toFixed(2) 
     }
     if(Number.isNaN(returnedJobavg) || returnedJobavg == Infinity){
      
      this.returnedJobavg= 0 
     }else{
      this.returnedJobavg= returnedJobavg.toFixed(2) 
     }
     if(Number.isNaN(totalIncomeavg)|| totalIncomeavg == Infinity){
      this.totalIncomeavg= 0 
     }else{
     
      this.totalIncomeavg= totalIncomeavg.toFixed(2) 
     } 
  }
  selectedDate:any={}
  custCount: any = [];
  ...
  getChart() {
    this.proService.getChart().subscribe((res: any) => {
      if (res.success) {
        for (let rc in res.result.revenueCounts) {
          this.revenueCount.push(res.result.revenueCounts[rc].count)
        }

        for (let jc in res.result.jobCounts) {
          this.jobCount.push(res.result.jobCounts[jc].count)
        }

        for (let cc in res.result.customerCounts) {
          this.custCount.push(res.result.customerCounts[cc].count)
        }
        this.createChart()
      }
    })
